...
CID 显示名来自公开派生目录 + 本机 pak `StringData.xml` 简体中文（`Zh_CN`）覆盖（见 `src/games/dragon-sword/catalog/SOURCE.md`）。未知 CID 仍显示为 `#CID`。
...
存档里物品/角色等用数字 **CID** 标识。编辑器已内置中英名称目录；若要自行核对或查找未显示的 ID，可用社区数据库：

- [th.gl · DragonSword: Awakening](https://dragonswordawakening.th.gl/)（角色、装备、料理、材料等分区）

应用内打开龙之剑模块时，编辑区顶部也有同款链接提示。
